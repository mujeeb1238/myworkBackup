...
function showMainContainer(containerDivName, username) {
	...
	if (jQuery.MifosXUI.showMenu("UserAdminMenu") == true || jQuery.MifosXUI.showMenu("OrgAdminMenu") == true || jQuery.MifosXUI.showMenu("SysAdminMenu") == true) {
		...
		if (jQuery.MifosXUI.showMenu("INVENTORYMENU") == true)    
		...
		if (jQuery.MifosXUI.showMenu("SCHEDULINGMENU") == true)    
			   htmlVar += '	<li><a href="unknown.html" onclick="refreshJobSchedulingDetails(' + "'" + 'content' + "'" + ');return false;">' + doI18N("link.topnav.schedule") + '</a></li>';
    ...
    htmlVar += '                <li><input type="checkbox" name="gsresource" value="LOANS"/><label for="loans">Orders</label></li>';
...
function setClientContent(divName) {
	...
	//htmlVar += '<li><a href="nothing" title="clientuploadtab" class="topleveltab"><span id="clientdocumenttabname">' + doI18N("client.uploadfile.tab.name")  + '</span></a></li>';
	//htmlVar += '<li><a href="nothing" title="clientuploadstatustab" class="topleveltab"><span id="clientdocumenttabname">' + doI18N("client.uploadstatusfile.tab.name")  + '</span></a></li>';
	...
	htmlVar += '<li><a href="nothing" title="clienttransactionhistorytab" class="topleveltab"><span id="clientdocumenttabname">' + doI18N("client.transactionhistory.tab.name")  + '</span></a></li>';
	//htmlVar += '<li><a href="nothing" title="inventorygrntab" class="topleveltab"><span id="clientdocumenttabname">' + doI18N("InventoryGrn")  + '</span></a></li>';
...
*/
	...
	if (jQuery.MifosXUI.showTask("PAYMODE") == true)
		 htmlOptions2 += '| <a href="unknown.html" onclick="addPaymode();return false;">' + doI18N("administration.link.paymode") + '</a>';
	...
	if (jQuery.MifosXUI.showTask("TAXMAP") == true)
			htmlOptions2 += '| <a href="unknown.html" onclick="refreshTableView(' + "'taxmap'" + ');return false;"" id="service" >' + doI18N("administration.link.taxmap.details") + '</a>';
...
function showILClient(clientId) {
	...
	var successFunction = function(data, status, xhr) {
				...
				//	executeAjaxRequest(clientUrl + '/loans', 'GET', "", successFunction, formErrorFunction);	  	
					...
					//	$("#addbtn").html('<br/><a id="newaddbtn" class="newaddbtn" href=# style=margin-right: 5px; onClick="addAddress('+clientId+');return false;">Add Address</a>');
					}
					else
						{
					...
					refreshLoanSummaryInfo(clientUrl);
				//	refreshBalanceSummaryInfo(clientUrl);
	      ...
	      var getUri="orders/"+clientId+"/orders";
			...
			var successFunction =  function(data, textStatus, jqXHR) {
				...
				$.each(crudObject.crudRows, function(i, val) {
				     ...
				     $("#reconnectOrder" + val.id).click(function(e) {
				     ...
				     $("#renewalorder" + val.id).click(function(e) {
		            		var entityId =val.id;
	...
	var clientId = clientUrl.replace("clients/", "");
      ...
      clientIdForOneTimesale = clientId;
		...
		var crudObject = new Object();
			...
			var iv = 0;
			for(;iv<data.length;iv++){
			$("#allocateHardware"+data[iv].id).click(function(e){
...
function refreshInventoryGrn(divName){
	...
	var successFunction =  function(data, textStatus, jqXHR) {
		...
		var getUrl='itemdetails/addgrn';
		  var postUrl='itemdetails/addgrn';//+clientId;
			...
			var saveSuccessFunction = function(data, textStatus, jqXHR) {
				$("#dialog-form").dialog("close");
				refreshInventoryGrn(divName);
			};
			
			popupDialogWithFormView(getUrl,postUrl, 'POST', "Grn Details", templateSelector, width, height,saveSuccessFunction);
		...
		var successFunction = function(data, textStatus, jqXHR) {
				...
				$("a.delete" + tableName).click( function(e) {
					...
					if (tableName === 'savingproduct' ||tableName === 'depositproduct' || tableName ==='charge' || tableName ==='user' || tableName == 'code' || tableName == 'officetransaction'|| tableName == 'report'
						|| tableName === 'item' || tableName === 'subscription' ) {
						var linkId = this.id;
	   ...
	   //Caledar form data
		...
		if(templateSelector == '#allocateHardwareFormListTemplate'){
    		...
    		for(;i<=quantityForOneTimeSale;i++){
    			serializedArray["serialNumber"] = $("#serialNumber"+i).val();
...
if(templateSelector == "#allocateHardwareFormListTemplate"){
	...
	for(;i<=quantity;i++){
		...
		$("table #appendSerialNumberInputField").append("<tr align="+"right"+"><td><label for="+"serialNumber"+i+""+
				">SerialNumber</label></td><td><input type="+"text"+" id="+"serialNumber"+i+""+" name="+"serialNumber"+i+""+" size="+"22"+" /></td></tr>");
	}
...
if (templateSelector === "#addressFormTemplate"){
	...
	$("#addEntityData").click(function(){
			...
			var errorFunction = function(jqXHR, status, errorThrown) {
				handleXhrError(jqXHR, status, errorThrown, 'addressFormTemplate', 'formerrors');
...
function addChargeCode() {
	maintainTable('chargecode', 'chargecodes', 'POST');
	return false;
}


function addTaxMap() {
	maintainTable('taxmap', 'taxmaps', 'POST');
	return false;
}
...
function launchOrderDialog(data){
					//var launchOrderDialogOnSuccessFunction = function(data, textStatus, jqXHR) {
						...
						var buttonsOpts = {};
						buttonsOpts[saveButton] = function() {
							submitTabbedOrder(dialogDiv);//need to update
						};
						// end of buttonsOpts for save button
					...
					//};
						//executeAjaxRequest('orders/template', 'GET', "", launchOrderDialogOnSuccessFunction, formErrorFunction);
				}
		// new code		
	function handleAddOnsTabSelection(data) {
	//var getServicesSuccessFunction = function(data, textStatus, jqXHR) {
		var baseObject = new Object();
		baseObject.services = data.mediaAttributes;
		var servicesSize = $('#services p').size();
...
//for media Details add button
		...
		//	  alert(servicesSize);
			...
			//make newly added select a combobox
			$("#" + baseObject.id).combobox({
		         selected: function(event, ui) {
		            var selectedServiceId = $("#" + baseObject.id).val();
		            $.each(servicesList, function(i, item) {
		                if (selectedServiceId == servicesList[i].id) {
		                	$("#" + baseObject.serviceType).val(servicesList[i].serviceUnitType);
		                	if (servicesList[i].serviceUnitType.toUpperCase() == "QUANTITY") {
		                		$("#" + baseObject.units).val(1); //update here for quantity
		                		$("#" + baseObject.units).removeAttr('readonly');
			                	$("#" + baseObject.price).val(servicesList[i].price);
			                	$("#" + baseObject.units).blur(function() {
			                		var object = new Object();
			                		$("#" + baseObject.price).val(servicesList[i].price);
				                	object.serviceType = servicesList[i].serviceUnitType;
				                	object.units = $("#" + baseObject.units).val();
				                	object.price = $("#" + baseObject.price).val();
				                	object.locale = "en";
				                	var successFunction = function(data, textStatus, jqXHR) {
				                		$("#" + baseObject.price).val(data);
									};
				                	executeAjaxRequest("orders/optioalServicePrice", "POST", JSON.stringify(object), successFunction, errorFunction);
			                	});
							}
		                	if (servicesList[i].serviceUnitType.toUpperCase() == "TEST SCHEME") {
		                		$("#" + baseObject.units).val(1); //update here for scheme
		                		$("#" + baseObject.units).removeAttr('readonly');
			                	$("#" + baseObject.price).val(servicesList[i].price);
			                	$("#" + baseObject.units).blur(function() {
			                		var object = new Object();
				                	...
				                	executeAjaxRequest("orders/template", "POST", JSON.stringify(object), successFunction, errorFunction);
			                	});
							}
		                	if (servicesList[i].serviceUnitType == "ON/OFF") {
		                		$("#" + baseObject.units).attr('readonly','readonly');
		                		$("#" + baseObject.units).val(1); //update here for scheme
			                	$("#" + baseObject.price).val(servicesList[i].price);
							}
						}
		...
		$("#"+baseObject.units).click(function(e) {
			//alert(baseObject.units);
		});	
			e.preventDefault();
		});
...
//for mediaLocation add button
		...
		icons : {
			...
			var baseObject = new Object();
			baseObject.services = data.mediaFormat;
			servicesSize = servicesSize + 1;
...
//	executeAjaxRequest('orders/template', 'GET', "", getServicesSuccessFunction, formErrorFunction);
}



function submitTabbedOrder(divContainer) {
