...
public class BillingMasterApiResourse {
	...
	//	final CommandWrapper commandRequest = new CommandWrapperBuilder().createBill(clientId).withJson(apiRequestBodyAsJson).build();
		
		 final JsonElement parsedCommand = this.fromApiJsonHelper.parse(apiRequestBodyAsJson.toString());
         
         final JsonCommand command = JsonCommand.from(apiRequestBodyAsJson.toString(),parsedCommand,
