...
public class OrderWritePlatformServiceImpl implements OrderWritePlatformService {
	...
	try{
			...
			for(OrderPrice str:order.getPrice()){
				orderPrice = orderPrice.concat(str.getPrice().toString()+" ");
			}
			String services = new String();
			for(OrderLine orderLine:order.getServices()){
				services = services.concat(orderLine.getServiceType()+" ");
			}
			...
			transactionHistoryWritePlatformService.saveTransactionHistory(order.getClientId(), "New Order", order.getStartDate(),"PlanId:"+order.getPlanId(),"Price:"+orderPrice,"contarctPeriod:"+order.getContarctPeriod(),"OrderID:"+order.getId(),"Services:"+services,"BillingAlign:"+order.getbillAlign());
	...
	public CommandProcessingResult deleteOrder(Long orderId, JsonCommand command) {
		...
		transactionHistoryWritePlatformService.saveTransactionHistory(order.getClientId(), "Delete Order", order.getEndDate(),"Price:"+orderPrice,"PlanId:"+order.getPlanId(),"contarctPeriod:"+order.getContarctPeriod(),"Services:"+services,"OrderID:"+order.getId(),"BillingAlign:"+order.getbillAlign());
	...
	public CommandProcessingResult updateOrder(JsonCommand command ) {
		try {
			...
			transactionHistoryWritePlatformService.saveTransactionHistory(order.getClientId(), "Update Order", order.getStartDate(),
					"Price:"+orderPrice,"PlanId:"+order.getPlanId(),"contarctPeriod:"+order.getContarctPeriod(),"services"+services,"OrderID:"+order.getId(),"BillingAlign:"+order.getbillAlign());
