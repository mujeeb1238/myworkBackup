...
public class BillingMessageApiResource {
	...
	private final Set<String> MESSAGE_DATA_PARAMETERS = new HashSet<String>(Arrays.asList("id", "templateDescription", 
			"subject", "header", "body","footer","messageParams"));
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String addData(@PathParam("clientId") final Long clientId,final String apiRequestBodyAsJson) {
		
		 final CommandWrapper commandRequest = new CommandWrapperBuilder().createMessageData(clientId).withJson(apiRequestBodyAsJson).build();
    ...
    public String retrieveSingleMessageTemplate(@PathParam("messageId") final Long messageId,@Context final UriInfo uriInfo) {
        ...
        List<BillingMessageData> messageparam=this.billingMesssageReadPlatformService.retrieveMessageParams(messageId);
                  templateData.setMessageParams(messageparam);
        final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
        return this.toApiJsonSerializer.serialize(settings, templateData, MESSAGE_DATA_PARAMETERS);
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String templateEditData(@PathParam("clientId") final Long clientId,final String apiRequestBodyAsJson) {
		
		 final CommandWrapper commandRequest = new CommandWrapperBuilder().updateMessageData(clientId).withJson(apiRequestBodyAsJson).build();
