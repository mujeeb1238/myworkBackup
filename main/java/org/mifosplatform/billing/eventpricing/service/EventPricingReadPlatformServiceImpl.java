...
public class EventPricingReadPlatformServiceImpl implements 
	...
	private static final class EventPricingMapper implements RowMapper<EventPricingData>  {
		...
		public String eventPricingSchema() {
			return " ep.id AS id,em.id AS eventId,em.event_name AS eventName,ep.format_type AS formatType,ep.opt_type AS optType, mc.code_value as clientType,"
				  +" d.discount_description AS discount,ep.price AS price " +
		...
		public EventPricingData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
			...
			return new EventPricingData(id, eventName, formatType, optType, clientType, discount, price,eventId);
		}
