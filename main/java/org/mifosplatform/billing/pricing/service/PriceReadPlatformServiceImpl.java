			...
			public List<ServiceData> retrievePriceDetails(String planCode) {
				  ...
				  context.authenticatedUser();
			        ...
			        String sql = "SELECT  p.plan_code as plan_code,pm.id as id,pm.service_code as service_code,se.service_description as service_description," +
						...
						"pm.price as price from  b_plan_master p,b_plan_pricing  pm,b_service se,b_charge_codes c where p.id=pm.plan_id and pm.service_code = se.service_code and" +
...
public List<ServiceData> retrieveServiceCodeDetails(Long planCode) {
	  ...
	  context.authenticatedUser();
        ...
        String sql = "SELECT p.id AS planId, pm.id AS id,ch.charge_description AS chargeDescription, pm.plan_id AS plan_code,"
			+"pm.service_code AS service_code,pm.charge_code AS charge_code  FROM b_plan_master p, b_plan_pricing pm,b_charge_codes ch"
...
public PricingData retrieveSinglePriceDetails(String priceId) {
     ...
     String sql = "SELECT p.plan_id as planId,s.id AS serviceId,c.id AS chargeId,p.charging_variant AS chargeVariant, p.price AS price, p.discount_id AS discountId"
                +" FROM b_plan_pricing p, b_service s,b_charge_codes c   where  p.charge_code = c.charge_code" +
