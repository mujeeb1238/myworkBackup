...
public class BillingOrderWritePlatformServiceImplementation implements BillingOrderWritePlatformService {
	...
	private final ProcessDate processDate;
	...
	public BillingOrderWritePlatformServiceImplementation(final PlatformSecurityContext context,
			...
			final ClientBalanceRepository clientBalanceRepository,final ProcessDate processDate
			) {
	...
	public List<BillingOrder> createBillingProduct(
		...
		if (billingOrderCommands.size() != 0) {
			...
			for (BillingOrderCommand billingOrderCommand : billingOrderCommands) {
				...
				//EntityManagerFactory  emf = Persistence.createEntityManagerFactory("b_charge");
