...
public class PlansApiResource {
	...
	private  final Set<String> RESPONSE_DATA_PARAMETERS = new HashSet<String>(Arrays.asList("id", "planCode", "plan_description", "startDate",
            "endDate", "status", "service_code", "service_description", "charge_code", "charge_description","servicedata","contractPeriod","provisionSystem",
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retrieveClientAccount(@QueryParam("plan_code") final String planCode,
	 ...
	 PlanData planData = new PlanData(data, billData, contractPeriod, status,null,null,provisionSysData);
	...
	public String retrieveAllSubscription(@Context final UriInfo uriInfo) {
 		 context.authenticatedUser().validateHasReadPermission(resourceNameForPermissions);
		List<PlanData> products = this.planReadPlatformService.retrievePlanData();
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
	...
	public String retrievePlanDetails(@PathParam("planId") final Long planId,@Context final UriInfo uriInfo) {
		...
		int selectedsize = services.size();
			...
			singlePlandata = new PlanData(data, billData, contractPeriod,status, singlePlandata, services,provisionSysData);
			final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
