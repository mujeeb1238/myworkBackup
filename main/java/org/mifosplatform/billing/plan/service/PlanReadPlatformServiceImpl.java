...
public class PlanReadPlatformServiceImpl implements PlanReadPlatformService {
	...
	private static final class BillRuleDataMapper implements
		...
		public String schema() {
			return "b.id as id,b.billing_rule as billingRule from b_billing_rules b;";
	...
	private static final class PlanDataMapper implements RowMapper<PlanData> {
		...
		public PlanData mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			EnumOptionData enumstatus=SavingStatusEnumaration.interestCompoundingPeriodType(planStatus);
	...
	public List<EnumOptionData> retrieveNewStatus() {
		EnumOptionData active = SavingStatusEnumaration.interestCompoundingPeriodType(StatusTypeEnum.ACTIVE);
		EnumOptionData inactive = SavingStatusEnumaration.interestCompoundingPeriodType(StatusTypeEnum.INACTIVE);
	...
	public PlanData retrievePlanData(Long planCode) {
		  context.authenticatedUser();
	        ...
	        String sql = "SELECT pm.id as id,pm.plan_code as plan_code,pm.plan_description as plan_description,pm.start_date as start_date,"
				+"pm.end_date as end_date,pm.plan_status as plan_status,pm.provision_sys as provisionSys,pm.bill_rule as bill_rule,pm.contract_period as contract_period "
				+"  from b_plan_master pm  Where pm.id=? and pm.is_deleted='n'";
	 ...
	 private static final class ServiceMapper implements RowMapper<PlanData> {
	        ...
	        Long id = rs.getLong("id");
	            ...
	            return new PlanData(id,planCode,startDate,endDate,billRule,contractPeriod,status1,planDescription,status1,provisionSys,null);
	        }
	}
