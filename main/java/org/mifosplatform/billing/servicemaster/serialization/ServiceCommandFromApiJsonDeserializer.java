...
public final class ServiceCommandFromApiJsonDeserializer {
    ...
    public void validateForCreate(final String json) {
        ...
        baseDataValidator.reset().parameter("serviceDescription").value(serviceDescription).notBlank().notExceedingLengthOf(100);
        final Integer  serviceType = fromApiJsonHelper.extractIntegerSansLocaleNamed("serviceType", element);
        baseDataValidator.reset().parameter("serviceType").value(serviceType).notBlank().notExceedingLengthOf(100).integerGreaterThanZero();
