...
public final class InventoryItemCommandFromApiJsonDeserializer {
	...
	private final Set<String> supportedParameters = new HashSet<String>(Arrays.asList("itemMasterId", "serialNumber", "grnId","provisioningSerialNumber", "quality", "status","warranty", "remarks","locale","officeId","clientId","inventorylisttable_length"));
    ...
    public void validateForCreate(final JsonCommand command) {
        ...
        if(fromApiJsonHelper.parameterExists("itemMasterId", element)){
        ...
        if(fromApiJsonHelper.parameterExists("warranty", element)){
        	final Long warranty = fromApiJsonHelper.extractLongNamed("warranty", element);
        	baseDataValidator.reset().parameter("warranty").value(warranty).notNull().notExceedingLengthOf(99);
        }
        
        if(fromApiJsonHelper.parameterExists("grnId", element)){
        	final Long grnId = fromApiJsonHelper.extractLongNamed("grnId", element);
