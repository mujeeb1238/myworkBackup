...
public class TaxMapReadPlatformServiceImp implements TaxMapReadPlatformService{
	...
	public List<TaxMapData> retriveTaxMapData() {
		...
		String sql = "select tmr.id as id, tmr.charge_code as chargeCode, tmr.tax_code as taxCode, tmr.start_date as startDate, tmr.type, tmr.rate as rate from b_tax_mapping_rate tmr";
	...
	public TaxMapData retriveTaxMapDataForUpdate(final Long id) {
		...
		String sql = "select tmr.id as id, tmr.charge_code as chargeCode, tmr.tax_code as taxCode, tmr.start_date as startDate, tmr.type, tmr.rate as rate from b_tax_mapping_rate tmr where tmr.id = ?";
		TaxMapDataMapper rowMapper = new TaxMapDataMapper();
	...
	private class TaxMapDataMapper implements RowMapper<TaxMapData>{
		...
		public TaxMapData mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			return new TaxMapData(id,chargeCode,taxCode,startDate,type,rate);
		}
