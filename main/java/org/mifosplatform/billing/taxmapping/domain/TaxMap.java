...
public class TaxMap extends AbstractPersistable<Long>{
	...
	public TaxMap(final String chargeCode, final String taxCode, final Date startDate,final String type,final BigDecimal rate){
	...
	public static TaxMap fromJson(JsonCommand command){
		...
		Date d = command.DateValueOfParameterNamed("startDate");
		String type = command.stringValueOfParameterNamed("type");
		BigDecimal rate = command.bigDecimalValueOfParameterNamed("rate");
		
		return new TaxMap(chargeCode,taxCode,startDate.toDate(),type,rate);
	}
