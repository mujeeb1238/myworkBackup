...
public final class ClientDataValidator {
    ...
    public void validateForUpdate(final String json) {
        ...
        if (fromApiJsonHelper.parameterExists(ClientApiConstants.clientCategoryParamName, element)) {
            atLeastOneParameterPassedForUpdate = true;
            final String clientId = fromApiJsonHelper.extractStringNamed(ClientApiConstants.clientCategoryParamName, element);
            if(clientId.contains("-1"))
            	baseDataValidator.reset().parameter(ClientApiConstants.clientCategoryParamName).value(clientId).integerGreaterThanZero();
            else
            	baseDataValidator.reset().parameter(ClientApiConstants.clientCategoryParamName).value(clientId).notExceedingLengthOf(100);
        }
